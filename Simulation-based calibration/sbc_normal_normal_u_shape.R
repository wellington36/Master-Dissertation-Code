...
# ERROR PARAMETERS: The key to the U-Shape
SIGMA_Y_SIM <- 1.0   # Data is simulated with this true standard deviation
SIGMA_Y_STAN <- 0.5  # Stan model is TOLD this standard deviation (The Error)
...
run_sbc_normal_normal <- function(mod, N_SIMS, N_OBS, L_SAMPLES, N_CHAINS, N_WARMUP) {
  ...
  for (s in 1:N_SIMS) {
    ...
    # Simulate data (y) from the likelihood (using the true sigma)
    y <- rnorm(N_OBS, mu_true, SIGMA_Y_SIM)
    
    data_list <- list(
      N = N_OBS, 
      y = y, 
      sigma_y = SIGMA_Y_STAN, # The Error: Stan is given the wrong sigma
...
# --- Run SBC ---
cat(paste("Starting SBC run for Case Study 2 with", N_SIMS, "simulations...\n"))

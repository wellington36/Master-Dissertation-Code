...
iter_warmup   <- 1000
iter_sampling <- 500
core_number   <- 50      # 3 for my machine 50 for virtual machine

# data
inventory <- read.csv("data/Shmuelli_2005.csv")


inventory_full <- data.frame(
  value = rep(inventory$count, times = inventory$frequency)
)


y_obs <- inventory_full$value
N <- length(y_obs)
